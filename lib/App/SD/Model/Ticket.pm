...
__PACKAGE__->register_reference(
    comments => 'App::SD::Collection::Comment',
    by       => 'ticket'
);
__PACKAGE__->register_reference(
    attachments => 'App::SD::Collection::Attachment',
    by          => 'ticket'
);
...
sub BUILD {
    my $self = shift;
    $ACTIVE_STATUSES ||=
      $self->app_handle->setting( label => 'active_statuses' )->get();
}

sub default_prop_component {
    my $self = shift;
    return $self->app_handle->setting( label => 'default_component' )->get()
      ->[0];
}

sub default_prop_milestone {
    my $self = shift;
    return $self->app_handle->setting( label => 'default_milestone' )->get()
      ->[0];
}
...
my %canonicalize_status = ( resolved => 'closed', );
...
If called with 'verbose' as a true value, will return all the declared props of
a ticket rather than the predefined list of ones to show. (Should not be called
this way during a ticket create as new tickets have no declared properties.)

If called with 'update' as a true value, props in the prop ordering setting
will still be returned in the list even if the record doesn't have that
property. (Because we often want to not show blank properties, but still have
the option of adding them in an update.)
...
sub props_to_show {
    ...
    return _create_prop_ordering(
        hash_to_order => $self->get_props,
        order         => $props_list,
        update        => $args->{update}
    );
}
...
Returns false if the date is not a valid ISO date or its due date is in the
future. Returns true if the due date has passed.
