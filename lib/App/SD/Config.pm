...
### completely.
    ...
    sub _old_app_config_file {
        ...
        my $old_file = $self->_file_if_exists( $ENV{'SD_CONFIG'} )
          || $self->_file_if_exists(
            File::Spec->catfile(
                $self->app_handle->handle->fs_root => 'config'
            )
          )
          || $self->_file_if_exists(
            File::Spec->catfile(
    ...
    override load => sub {
        ...
        for my $file (
            (
                $self->_old_app_config_file, $self->dir_file,
...
### the following methods.
    ...
    our %KEYS_CONVERSION_TABLE = (
        ...
        'disable_ticket_show_history_by_default' =>
          'ticket.no-implicit-history-display',
    );
