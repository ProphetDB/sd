...
sub format_change {
    ...
    my $output =
      $args{header_callback} ? $args{header_callback}->( $args{change} ) : '';

    if (   $args{change}->record_type eq 'comment'
        && $args{change}->change_type eq 'add_file' )
    {
        ...
        $output .= App::SD::CLI::Command::Ticket::Show->format_comment(
            (
                (
                         $change->{prop_changes}->{'content_type'}
        ...
        if ( my @prop_changes = $args{change}->prop_changes ) {
            $output .= $args{change_header}->( $args{change} )
...
sub format_prop_changes {
    ...
    for (@$prop_changes) {
        if ( defined $_->new_value && defined $_->old_value ) {
            $output .=
                sprintf( "%18.18s", $_->name )
              . ": changed from "
              . $_->old_value . " to "
              . $_->new_value;
        } elsif ( defined $_->new_value ) {
            $output .=
              sprintf( "%18.18s", $_->name ) . ": set to " . $_->new_value;
