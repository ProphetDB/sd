...
sub integrate_ticket_update {
    ...
    my $ticket =
      Net::Trac::Ticket->new( connection => $self->sync_source->trac );
    $ticket->load($remote_ticket_id)
      or die "couldn't load remote track ticket $remote_ticket_id\n";
    $ticket->update( %{ $self->_recode_props_for_integrate($change) } )
      or die "couldn't update remote track ticket $remote_ticket_id\n";
...
sub integrate_attachment {
    ...
    close $fh;
    $ticket->attach( file => $file )
      || die "Could not attach file for ticket $ticket_id";
    return $ticket_id;
}
