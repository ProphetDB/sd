...
sub translate_ticket_state {
    ...
    my $ticket_data   = {
        ...
        created => (
            $ticket_object->created->ymd . " " . $ticket_object->created->hms
        ),
...
sub find_matching_tickets {
    ...
    if ($last_changeset_seen_dt) {
        ...
        @results =
          grep { $_->last_modified >= $last_changeset_seen_dt } @results;
    }
    return \@results;
}
...
sub transcode_one_txn {
    ...
    foreach my $prop_change ( values %{ $txn->prop_changes || {} } ) {
        ...
        if (
            ( !defined $new && !defined $ticket->{$property} )
            || (   defined $new
                && defined $ticket->{$property}
                && $ticket->{$property} eq $new )
          )
        {
            $ticket->{$property} = $old;
        }
