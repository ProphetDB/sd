...
sub translate_ticket_state {
    ...
    $ticket->{$_} = undef
      for grep defined $ticket->{$_} && $ticket->{$_} eq '',
      keys %$ticket;
    ...
    $ticket->{'Owner'} =
      $self->resolve_user_id_to( email_address => $ticket->{'Owner'} )
      if $ticket->{'Owner'};
    ...
    for my $date ( grep defined $ticket->{$_},
...
Returns an RT::Client ticket collection for all tickets found matching your
QUERY string.
...
sub find_matching_tickets {
    ...
    if ( my $before = $self->_only_pull_tickets_modified_after ) {
        ...
        $self->sync_source->log(
            "Skipping all tickets not updated since " . $before->iso8601 );
    }
    return [
        map {
            ...
            my $hash =
              $self->sync_source->rt->show( type => 'ticket', id => $_ );
            $hash->{id} =~ s|^ticket/||g;
            $hash
...
sub find_matching_transactions {
    ...
    for my $txn (
        sort $rt_handle->get_transaction_ids( parent_id => $ticket_id ) )
    {
        ...
        next
          if $self->sync_source->foreign_transaction_originated_locally( $txn,
            $ticket_id );
        ...
        push @txns,
          {
            timestamp =>
              App::SD::Util::string_to_datetime( $txn_hash->{Created} ),
            serial => $txn_hash->{id},
            object => $txn_hash
          };
    }
...
sub transcode_one_txn {
    ...
    unless ($sub) {
        die
          "Transaction type $txn->{Type} (for transaction $txn->{id}) not implemented yet";
    }
    my $changeset = Prophet::ChangeSet->new(
        {
            ...
            created              => $txn->{'Created'},
            creator =>
              $self->resolve_user_id_to( email_address => $txn->{'Creator'} ),
        }
    );
...
{    # Recoding RT transactions
    ...
    sub _recode_txn_Set {
        ...
        my ( $field, $old, $new ) =
          @{ $args{txn} }{qw(Field OldValue NewValue)};

        if ( $field eq 'Queue' ) {
            my $current_queue =
              $args{'ticket'}->{ $self->sync_source->uuid . '-queue' };
            my $user = $args{txn}->{Creator};
            if ( $args{txn}->{Description} =~
    ...
    *_recode_txn_Give  = \&_recode_txn_Set;
    ...
    sub _recode_txn_Create {
        ...
        $self->_recode_content_update(%args)
          ; # add the create content txn as a seperate change in this changeset
    }
    ...
    *_recode_txn_Correspond = \&_recode_content_update;
    ...
    sub _recode_txn_AddWatcher {
        ...
        $args{'ticket'}->{ $type . 's' } = $self->warp_list_to_old_value(
            $new_state,
            $self->resolve_user_id_to(
                email_address => $args{'txn'}->{'NewValue'}
            ),
            $self->resolve_user_id_to(
                email_address => $args{'txn'}->{'OldValue'}
            )
        );
...
sub resolve_user_id_to {
    ...
    my $user = eval {
        ...
        local $SIG{__WARN__} = sub { $self->sync_source->log_debug(@_) };
        RT::Client::REST::User->new( rt => $self->sync_source->rt, id => $id )
          ->retrieve;
    };
    if ( my $err = $@ ) {
        warn $err;
