...
Once a ticket is created, the following modifications can be made to it: - edit
ticket body/title - add a new comment - edit a comment's body - close a ticket
(or reopen)
...
sub BUILD {
    ...
    if ( defined $access_token ) {
        $self->github( Net::GitHub->new( access_token => $access_token, ) );
    } else {
        $self->login_loop(
            uri            => $uri,
...
sub _uuid_url {
    ...
    Carp::cluck "- can't make a uuid for this"
      unless ( $self->remote_url && $self->owner && $self->repo );
