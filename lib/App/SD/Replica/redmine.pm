...
sub BUILD {
    ...
    my ( $server, $type, $query ) = $self->{url} =~ m/^redmine:(.*?)$/
      or die
      "Can't parse Redmine server spec. Expected something like redmine:http://example.com/projects/project_name\n";
    ...
    ( $username, $password ) = $self->prompt_for_login(
        uri      => $uri,
        username => $username,
    ) unless $password;
