...
Immutable ticket metadata includes things such as ticket id and creation date
that are useful to display to the user when editing a ticket but are
automatically assigned by sd and are not intended to be changed manually.

=cut

use constant metadata_separator =>
...
using L<Prophet::CLI::TextEditorCommand->try_to_edit> and then parsed using
L</parse_record_template>.

If RECORD is given, then we are updating that record rather than creating a new
one, and the ticket string will be created from its props rather than prop
defaults.
      ...
      )
    {
        if ( $do_not_edit{$prop} ) {
            if ( $prop eq 'id' && $update ) {
                ...
                !( ( $prop eq 'id' or $prop eq 'created' ) && !$update ) )
            {
                push @metadata_order, $prop;
...
sub _build_kv_pairs {
    ...
    for my $prop ( @{ $args{order} } ) {
        ...
        if ( $args{verbose} ) {
            if ( my $desc =
                $self->app_handle->setting( label => 'prop_descriptions' )
                ->get()->[0]->{$prop} )
            {
                $string .= '# ' . $desc . "\n";
            }
