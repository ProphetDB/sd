...
under help => sub {
    ...
    on [ [ 'attach', 'attachment', 'attachments' ] ] =>
      run_command('Help::Attachments');
    ...
    on [ [ 'ticket', 'attachment', 'comment' ],
        [ 'list', 'search', 'find' ], ] => run_command('Help::Search');
    ...
    on qr/^(\S+)$/ => sub {
        my $self  = shift;
        my $topic = $1;
        die "Cannot find help on topic '$topic'. Try '"
          . $self->cli->get_script_name()
          . "help'?\n";
    };
};
...
on qr'.*' => sub {
    ...
    print join( "\n",
        ...
        qq{Type "}
          ...
          . qq{help init" or "}
          . $self->cli->get_script_name()
          . qq{help environment" for tips on how to sort that out.} );
    exit 1;
};
...
under ticket => sub {
    ...
    on
      qr/^((?:comment\s+)?(?:comments?|update|edit|show|details|display|delete|del|rm|history|claim|take|resolve|basics|close)) $Prophet::CLIContext::ID_REGEX$/i
      => sub {
        my $self = shift;
    ...
    under attachment => sub {
        ...
        on [ [ 'create', 'new' ], qr/^$Prophet::CLIContext::ID_REGEX$/ ] =>
          sub {
            my $self = shift;
            $self->context->set_id_from_primary_commands;
...
on qr/^(.*)$/ => sub {
    ...
    my $command = $1;
    die
      "The command you ran, '$command', could not be found. Perhaps running '"
      . $self->cli->get_script_name
      . "help' would help?\n";
