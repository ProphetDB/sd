...
sub run {
    ...
    my $tickets =
      $self->find_matching_tickets( query => $self->sync_source->query );
    ...
    for my $ticket (@$tickets) {
        ...
        my $changesets;
        $progress->();
        $self->sync_source->log_debug(
            "Fetching $counter of " . scalar @$tickets . " tickets" );
        ( $last_modified, $changesets ) =
          $self->transcode_ticket( $ticket, $last_modified );
        unshift @changesets, @$changesets;
    }
    my $sorted_changesets =
      [ sort { $a->original_sequence_no <=> $b->original_sequence_no }
          @changesets ];
...
sub transcode_ticket {
    ...
    my $changesets;
    ( $last_modified, $changesets ) =
      $self->transcode_history( $ticket, $transactions, $last_modified );
...
sub transcode_history {
    ...
    for my $txn ( sort { $b->{'serial'} <=> $a->{'serial'} } @$transactions ) {
        ...
        my $changeset =
          $self->transcode_one_txn( $txn, $initial_state, $final_state );
...
If we've previously pulled from this sync source, this routine will return a
datetime object. It's safe not to evaluate any ticket last modified before that
datetime
